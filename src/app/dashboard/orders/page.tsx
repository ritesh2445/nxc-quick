...
import { ShoppingBag, Package, Check, Download, ExternalLink, Clock, CheckCircle } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/[0.08]">
        <div>
          <span className="font-mono text-xs text-[#A1A1AA] uppercase tracking-[0.2em] font-medium flex items-center gap-1.5">
            <Package className="w-3.5 h-3.5 text-[#C8C6C0]" /> COMMISSIONS & INVOICES
          </span>
        ...
        <Link href="/order" className="w-full sm:w-auto btn-interactive shrink-0">
          <Button variant="primary" size="sm" className="w-full sm:w-auto text-xs min-h-[44px] tracking-wider rounded-full bg-white text-black hover:bg-[#E5E5EA]">
      ...
      <div className="bg-[#0E0E12] border border-white/[0.08] rounded-2xl p-4 sm:p-6 space-y-4 shadow-xl">
        <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
          <h3 className="font-cinzel font-medium text-sm sm:text-base text-white tracking-wider flex items-center gap-2">
            <Package className="w-4 h-4 text-[#C8C6C0]" />
            Order Ledger
          </h3>
        ...
        {orders.length === 0 ? (
          <div className="text-center py-14 space-y-4 px-4">
            ...
            <Link href="/order">
              <Button variant="primary" size="sm" className="text-xs min-h-[44px] rounded-full bg-white text-black hover:bg-[#E5E5EA]">
                Order a Card
              </Button>
            </Link>
          </div>
              ...
              {orders.map((ord: any) => (
                <div key={ord.id} className="py-4 space-y-2.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-[10px] uppercase px-2.5 py-0.5 rounded-full bg-emerald-950/30 text-emerald-400 border border-emerald-800/30">
              ...
              <table className="w-full text-left text-xs font-sans border-collapse">
                <thead>
                  <tr className="border-b border-white/[0.08] text-[#8E8E98] font-mono text-[10px] uppercase tracking-wider">
                    ...
                    <th className="pb-3 px-3">Hardware Edition</th>
                    <th className="pb-3 px-3">Engraving Line</th>
                    <th className="pb-3 px-3">Amount</th>
                    <th className="pb-3 px-3">Status</th>
                    <th className="pb-3 px-3 text-right">Commission Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.04]">
                  {orders.map((ord: any) => (
                    <tr key={ord.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="py-4 px-3 font-mono text-white font-medium">
                        {ord.orderNumber}
                      </td>
                      <td className="py-4 px-3 capitalize text-white">
                        {ord.tier} · {ord.finish.replace("_", " ")}
                      </td>
                      <td className="py-4 px-3 font-mono text-[#C8C6C0]">
                        {ord.engravingName}
                      </td>
                      <td className="py-4 px-3 font-semibold text-white font-mono">
                        {formatCurrency(ord.amount, ord.currency as "INR" | "USD")}
                      </td>
                      ...
                      <td className="py-4 px-3 text-right font-mono text-[11px] text-[#70707C]">
                        {formatDate(ord.createdAt)}
                      </td>
                    </tr>
                  ))}
