...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white flex flex-col md:flex-row pt-14 md:pt-16 relative overflow-x-hidden">
      ...
      {/* ========================================================================= */}
      {/* MOBILE RESPONSIVE SUB-HEADER & HORIZONTAL LUXURY PILL NAV (Screens < md)   */}
      {/* ========================================================================= */}
      <div className="md:hidden sticky top-14 z-30 w-full bg-[#050508]/90 backdrop-blur-2xl border-b border-white/[0.08] shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
        {/* Top Mobile Bar: User handle + Live link */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.05]">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[9px] text-[#00A2FF] uppercase tracking-[0.18em] px-2 py-0.5 rounded-full bg-[#0055FF]/15 border border-[#0099FF]/30">
              CONSOLE
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link
              href={`/@${username}`}
              target="_blank"
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/[0.05] border border-white/15 text-[10px] font-sans font-medium text-[#D0D0DC] hover:text-white transition-all btn-interactive"
            >
        ...
        {/* Horizontally Scrollable Luxury Pill Navigation */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-sans font-medium transition-all duration-200",
                  isActive
                    ? "bg-gradient-to-r from-[#0055FF]/50 via-[#0077FF]/40 to-[#00A2FF]/30 text-white border border-[#0099FF]/60 shadow-[0_0_12px_rgba(0,140,255,0.35)]"
                    : "text-[#9E9EA8] hover:text-white bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.08]"
                )}
              >
                <Icon className={cn("w-3.5 h-3.5", isActive ? "text-[#00A2FF]" : "text-[#7E7E8E]")} />
      ...
      {/* ========================================================================= */}
      {/* DESKTOP SIDEBAR NAVIGATION (Screens >= md)                                 */}
      {/* ========================================================================= */}
      <aside className="hidden md:flex w-64 bg-[#050508]/80 border-r border-white/[0.08] backdrop-blur-2xl flex-col justify-between shrink-0 p-6 text-left relative z-10">
